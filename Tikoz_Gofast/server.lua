...
AddEventHandler("TikozAlertPopo", function()

	local xPlayers = ESX.GetPlayers()
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		if thePlayer.job.name == 'police' then
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], "Centrale LSPD", nil, "BMW immatriculé ~y~EX15AR ~w~est partie du ~b~port~w~ pour faire un ~r~Go Fast~w~ !", "CHAR_CALL911", 8)
...
AddEventHandler("TikozAlertCamtar", function()

	local xPlayers = ESX.GetPlayers()
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		if thePlayer.job.name == 'police' then
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], "Centrale LSPD", nil, "Un camion immatriculé ~b~AX87DV ~w~ est partie du ~y~port~w~ pour faire un ~r~Go Fast ~w~!", "CHAR_CALL911", 8)
